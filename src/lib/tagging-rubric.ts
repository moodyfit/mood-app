...
/**
 * 이미지 1장 채점 지시문(비전 모델용).
 * axisHint = 파일명 접두사(있으면 소프트 힌트). gender = "m"|"w"(캡션도 이 기준으로 맞춤).
 * gender 파라미터는 케빈의 여성 루브릭 작업(v3, 남녀 각 60장)을 SSOT에 통합한 것 — 회의록
 * "여성 루브릭 <케빈> → AI 계산 로직 반영 <에린>" 항목. AXIS_RUBRIC 자체는 성별 무관 어휘라 안 바꿈.
 */
