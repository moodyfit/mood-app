...
- [ ] 태깅 API에 `storage_path`(상대 경로)·`aspect_ratio`·`gender`·`photo_type`·`user_description`이 전달된다
- [ ] `Authorization: Bearer` 헤더에 세션 토큰이 실린다
- [ ] 짧은 변 `MIN_IMAGE_PX` 미만 사진은 업로드 전에 거절된다
- [ ] 업로드 중·채점 중 상태가 **사진 위에** 보이고, 완료 시 토스트가 뜬다
...
### 2026-09-18: 실사용 확인에서 나온 것

브라우저로 남성복·여성복을 각각 올려 결과를 대조했다.

- **남성복(전신 착장)은 정상 채점** — 네이비 헨리넥 니트·다크그레이 데님·블랙 레더 슈즈를 정확히 읽고 `classic 0.6 / clean 0.4`. 아이템·무드·캡션 모두 사진과 일치
- **여성복(플랫레이)은 한계 재확인** — 아이템 인식은 맞지만 `AXIS_RUBRIC`이 남성복 어휘뿐이라 무드는 추정에 가깝다. 루브릭 티켓 대상
- **`body_spec.height`를 지어낸다** — 사진으로 키를 알 수 없는데 두 건 모두 `"180"`. 루브릭이 필수로 요구하는 구조 문제라 루브릭 티켓 대상
- **UI 상태 피드백이 없었다** — 버튼 라벨만 바뀌는데 사진이 세로로 길어 버튼이 화면 밖으로 밀린다. 유저는 아무 반응 없다고 느끼고, 완료 후 예고 없이 상세로 이동한다. → 미리보기 위 오버레이 + 완료 토스트로 해결(이 티켓)
- **상품 섹션 문구가 사실과 다르다** — 연결 상품이 없으면 무드 폴백이 나오는데 제목은 `이 사진 그대로`. 시드 122장 중 90장만 연결돼 있어 업로드분만의 문제가 아니다. → 별도 티켓(Scope 제외 참고)

`PR #20`(케빈, v3 이미지 교체)이 성별 태깅을 다루지만 `AXIS_RUBRIC` 자체는 바꾸지 않아 업로드 채점 품질과는 무관하다. 또 그 PR은 `gender`를 `body_spec.gender`에 넣는데 이 티켓·FEAT-011은 `photos.gender` 컬럼을 쓴다 — 저장 위치 통일이 필요하다.
